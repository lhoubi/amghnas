document.addEventListener('DOMContentLoaded', () => {
    ...
    keyboardInput.addEventListener('keydown', (e) => {
        ...
        // If uppercase and no special mapping, fall back to lowercase equivalent
        if (!tifinaghChar && key.length === 1 && key === key.toUpperCase() && key !== key.toLowerCase()) {
    ...
    // --- Input event: converts ALL typed chars to Tifinagh (handles mobile + desktop) ---
    function convertToTifinagh(val) {
        let converted = '';
        let i = 0;
        while (i < val.length) {
            // Already Tifinagh? Keep it
            const code = val.charCodeAt(i);
            if (code >= 0x2D30 && code <= 0x2D7F) { converted += val[i++]; continue; }
            // Space? Keep it
            if (val[i] === ' ') { converted += ' '; i++; continue; }
            // Check digraphs first
            const two = val.slice(i, i + 2).toLowerCase();
            if (keyMap[two]) { converted += keyMap[two]; i += 2; continue; }
            const ch = val[i];
            const lower = ch.toLowerCase();
            // Arabic or special char?
            if (keyMap[ch]) { converted += keyMap[ch]; i++; continue; }
            // Latin lowercase
            if (keyMap[lower]) { converted += keyMap[lower]; i++; continue; }
            // Skip anything else
            i++;
        }
        return converted;
    ...
    keyboardInput.addEventListener('input', (e) => {
        ...
        const converted = convertToTifinagh(val);
        if (converted !== val) {
            keyboardInput.value = converted;
            const newPos = Math.min(pos, converted.length);
